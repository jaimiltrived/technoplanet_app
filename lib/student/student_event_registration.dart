...
class _StudentEventRegistrationScreenState
  ...
  int get _effectiveMaxTeamSize =>
      widget.event.maxTeamSize > 1 ? widget.event.maxTeamSize : 5;
  int get _effectiveMinTeamSize =>
      widget.event.minTeamSize > 1 ? widget.event.minTeamSize : 2;
  ...
  void initState() {
    ...
    if (_isTeamMode) {
      ...
      // Add 1 default group member safely during initState without triggering ScaffoldMessenger or setState
      final maxExtraMembers = _effectiveMaxTeamSize - 1;
  ...
  void _addGroupMember({bool showWarning = true}) {
    final maxMembers = _effectiveMaxTeamSize - 1; // excluding leader
    if (_groupMembers.length < maxMembers) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Maximum team size is $_effectiveMaxTeamSize members.'),
  ...
  void _removeGroupMember(int index) {
    if (_groupMembers.length > 1 || !widget.event.isTeamEvent) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Minimum $_effectiveMinTeamSize members required for this team event.'),
          backgroundColor: Colors.orange,
        ),
      );
    }
  ...
  Widget _buildEventSummary() {
    return Container(
      ...
      child: Row(
        children: [
          if (widget.event.hasBannerImage)
            ClipRRect(
                  ...
                  children: [
                    ...
                    if (widget.event.isTeamEvent) ...[
                      ...
                      Container(
                        ...
                        child: Text(
                          '👥 Team Event ($_effectiveMinTeamSize-$_effectiveMaxTeamSize Members)',
  ...
  Widget _buildGroupMembersSection() {
    final maxMembers = _effectiveMaxTeamSize - 1;
      ...
      children: [
        ...
        if (_groupMembers.length < maxMembers)
          OutlinedButton.icon(
            ...
            label: Text(
              'Add Team Member (${_groupMembers.length + 1} of $_effectiveMaxTeamSize)',
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        onPressed: _agreeTerms
            ? () {
                final dl = widget.event.registrationDeadline;
