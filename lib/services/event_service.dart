...
class EventService {
  ...
  static Future<Map<String, dynamic>> registerForEvent(String eventId, {Map<String, dynamic>? formData}) async {
    ...
    final isTeam = formData != null && (formData['isTeamRegistration'] == true || formData['isTeam'] == true);
    if (isTeam) {
      payload['isTeam'] = true;
      payload['isTeamRegistration'] = true;
      if (formData['teamName'] != null) payload['teamName'] = formData['teamName'];
      if (formData['groupMembers'] != null) {
        payload['groupMembers'] = formData['groupMembers'];
        payload['teamMembers'] = formData['groupMembers'];
      }
    }

    final url = isTeam ? ApiConfig.eventRegisterTeam : ApiConfig.eventRegister;
    final res = await ApiService.post(url, payload);
